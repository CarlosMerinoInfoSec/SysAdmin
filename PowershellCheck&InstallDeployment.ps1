<#
.SYNOPSIS
    This script will check to see if [software] is installed if not an attempt to copy [software] MSI file from a 
    network share to a local directory, and installation of [software] will be done.
.DESCRIPTION
    Test locally and ensure the script runs as intended before automating deployments.
    Checks to see if [software] is installed if not then checks to see if the [software] MSI file is on the local 
...
.NOTES
    File Name       [software]Check&Install.ps1
...
If($installed) {
    ...
    Start-Process -FilePath	msiexec.exe -ArgumentList '/i "C:\Install\[File].msi" /qn' -Wait -PassThru -ErrorAction Stop
